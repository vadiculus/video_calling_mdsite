...
let clientSocket = new WebSocket('ws://'+window.location.host+'/ws/premium_chat/'+chat_id+'/');
...
function onClose(){
    ...
    connectionInterval = setInterval(()=>{
        if (clientSocket.readyState === WebSocket.CLOSED){
            clientSocket = new WebSocket('ws://'+window.location.host+'/ws/premium_chat/'+chat_id+'/');
            clientSocket.onopen = onOpen;
            clientSocket.onclose = onClose;
            clientSocket.onmessage = onMessage;
        } else {
